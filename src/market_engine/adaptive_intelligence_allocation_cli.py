...
def _evaluate(args: argparse.Namespace) -> int:
    ...
    print("\nADAPTIVE INTELLIGENCE ALLOCATION ENGINE — COMPARACIÓN")
    ...
    if args.export:
        ...
        with pd.ExcelWriter(output, engine="openpyxl") as writer:
            ...
            _excel_safe(result.policy_comparison).to_excel(
                writer, sheet_name="Comparacion_Politicas", index=False
            )
            ...
            _excel_safe(result.allocation_by_date).to_excel(
                writer, sheet_name="Asignacion_Inteligencias", index=False
            )
            _excel_safe(result.intelligence_states).to_excel(
                writer, sheet_name="Estados_Inteligencias", index=False
            )
            _excel_safe(result.champion_scores).to_excel(
                writer, sheet_name="Scores_Actuales", index=False
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="market-intelligence-allocation")
    parser.add_argument(
    ...
    parser.add_argument("--output", default="adaptive_intelligence_allocation_v095.xlsx")
