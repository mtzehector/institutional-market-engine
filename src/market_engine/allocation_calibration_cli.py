...
def _evaluate(args: argparse.Namespace) -> int:
    ...
    print("\nALLOCATION CALIBRATION LABORATORY — COMPARACIÓN")
    if result.policy_comparison.empty:
        print("No hubo fechas suficientes para comparar curvas.")
    else:
    ...
    if args.export:
        ...
        with pd.ExcelWriter(output, engine="openpyxl") as writer:
            ...
            _excel_safe(result.policy_comparison).to_excel(
                writer, sheet_name="Comparacion_Curvas", index=False
            )
            _excel_safe(result.recommendations).to_excel(
                writer, sheet_name="Recomendaciones", index=False
            )
            _excel_safe(result.calibration_report).to_excel(
                writer, sheet_name="Calibration_Report", index=False
            )
            _excel_safe(result.conviction_report).to_excel(
                writer, sheet_name="Conviction_Report", index=False
            )
            _excel_safe(result.disagreement_report).to_excel(
                writer, sheet_name="Disagreement_Report", index=False
            )
            _excel_safe(result.allocation_by_date).to_excel(
                writer, sheet_name="Allocation_Functions", index=False
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="market-allocation-calibration")
    parser.add_argument(
    ...
    parser.add_argument("--output", default="allocation_calibration_v0951.xlsx")
