...
def _evaluate(args: argparse.Namespace) -> int:
    ...
    print("\nCHAMPION RESILIENCE ENGINE — ESTADO ACTUAL")
    if result.current_resilience.empty:
        print("No hubo datos suficientes para estimar resiliencia.")
    else:
        columns = [
            "champion",
            "lifecycle_state",
        ...
        print(result.current_resilience[columns].to_string(index=False))
    ...
    if args.export:
        ...
        with pd.ExcelWriter(output, engine="openpyxl") as writer:
            ...
            _excel_safe(result.current_resilience).to_excel(
                writer, sheet_name="Resiliencia_Actual", index=False
            )
            _excel_safe(result.recovery_episodes).to_excel(
                writer, sheet_name="Episodios_Recuperacion", index=False
            )
            _excel_safe(result.resilience_history).to_excel(
                writer, sheet_name="Historial_Resiliencia", index=False
            )
            _excel_safe(result.champion_summary).to_excel(
                writer, sheet_name="Resiliencia_Campeon", index=False
            )
            _excel_safe(result.family_summary).to_excel(
                writer, sheet_name="Resiliencia_Familia", index=False
            )
            _excel_safe(result.evidence).to_excel(
                writer, sheet_name="Evidencia_Resiliencia", index=False
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="market-champion-resilience")
    ...
    parser.add_argument("--output", default="champion_resilience_v098.xlsx")
