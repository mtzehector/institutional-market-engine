from __future__ import annotations

import pandas as pd

from market_engine.evaluation.champion_intelligence_council import (
    COUNCIL_POLICIES,
    run_champion_intelligence_council,
)


def _selections(days: int = 30, tickers: int = 16) -> pd.DataFrame:
    dates = pd.date_range("2026-01-02", periods=days, freq="B")
    champions = ["CHAMPION_A", "CHAMPION_B", "UNIVERSE"]
    rows: list[dict[str, object]] = []
    for day_index, date in enumerate(dates):
        for champion in champions:
            for ticker_index in range(tickers):
                rare = ticker_index % 4 == 0
                actual = "GAP_UP" if rare else "SIN_GAP"
                if champion == "CHAMPION_A":
                    good = day_index < 9 or day_index >= 21
                elif champion == "CHAMPION_B":
                    good = 7 <= day_index < 24
                else:
                    good = ticker_index % 2 == 0
                predicted = actual if good else "SIN_GAP"
                rows.append(
                    {
                        "origin_date": date,
                        "target_date": date + pd.offsets.BDay(1),
                        "champion": champion,
                        "ticker": f"T{ticker_index:02d}",
                        "actual_gap_pct": 1.5 if rare else 0.1,
                        "actual_gap_up": int(rare),
                        "actual_gap_down": 0,
                        "actual_direction": actual,
                        "predicted_direction": predicted,
                        "correct_direction": predicted == actual,
                        "probability_up": 0.78 if predicted == "GAP_UP" else 0.08,
                        "probability_down": 0.04,
                        "probability_no_gap": 0.18 if predicted == "GAP_UP" else 0.88,
                    }
                )
    return pd.DataFrame(rows)


def _states(days: int = 30) -> pd.DataFrame:
    return pd.DataFrame(
        {
            "origin_date": pd.date_range("2026-01-02", periods=days, freq="B"),
            "regime": ["RISK_ON_BROAD"] * 15 + ["ROTATION_VOLATILE"] * 15,
        }
    )


def test_council_builds_all_policies_and_engine_votes() -> None:
    result = run_champion_intelligence_council(
        _selections(),
        _states(),
        minimum_own_episodes=1,
        minimum_family_episodes=1,
        minimum_completed_spells=1,
    )
    assert not result.council_current.empty
    assert not result.engine_votes.empty
    assert not result.explanations.empty
    assert not result.disagreements.empty
    assert set(result.council_current["policy"]) == set(COUNCIL_POLICIES)
    assert result.council_current["council_authority_score"].between(0, 100).all()
    assert result.council_current["council_confidence"].between(0, 100).all()
    assert result.council_current.groupby("policy")["champion"].nunique().eq(2).all()


def test_council_is_auditable_and_veto_is_conservative() -> None:
    result = run_champion_intelligence_council(
        _selections(),
        _states(),
        minimum_own_episodes=1,
        minimum_family_episodes=1,
        minimum_completed_spells=1,
    )
    votes_per_champion = result.engine_votes.groupby(["policy", "champion"])["engine"].nunique()
    assert (votes_per_champion == 6).all()
    conservative = result.council_current.loc[
        result.council_current["policy"] == "CONSERVATIVE"
    ]
    exploratory = result.council_current.loc[
        result.council_current["policy"] == "EXPLORATORY"
    ]
    assert conservative["risk_veto"].sum() >= exploratory["risk_veto"].sum()
    assert result.summary.iloc[0]["policies"] == 3
